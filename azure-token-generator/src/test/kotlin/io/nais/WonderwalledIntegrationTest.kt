...
class WonderwalledIntegrationTest {
    private fun azureTokenGenerator(block: suspend ApplicationTestBuilder.() -> Unit) =
        testWonderwalled(Application::azureTokenGenerator) {
            block()
        }
    ...
    fun `POST api public obo returns raw access token on success`() =
        azureTokenGenerator {
            ...
            assertEquals(HttpStatusCode.OK, response.status)
            assertEquals("SOME_TOKEN", body)
        }

    @Test
    fun `POST api public m2m missing aud returns 400`() =
        azureTokenGenerator {
            val response = client.submitForm("/api/public/m2m")
            assertEquals(HttpStatusCode.BadRequest, response.status)
        }
}
